<template>
  <div>
    <b-form-checkbox :checked="isAllSelected" @change="onToggleAllChanged"
      >Select/Deselect All</b-form-checkbox
    >
    <b-list-group>
...
export default class TabList extends Vue {
  ...
  onItemChanged(tabId: string, checked: boolean): void {
  ...
  onToggleAllChanged(checked: boolean): void {
    if (checked) {
      this.selectAllTabs()
    } else {
